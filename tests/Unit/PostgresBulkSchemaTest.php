...
class PostgresBulkSchemaTest extends TestCase
{
    ...
    public function testAssembleColumnsEmitsNonDefaultIdentityOptions(): void
    {
        $rows = [self::colRow('t', 'id', [
            'data_type'           => 'bigint',
            'is_identity'         => 'YES',
        ...
        $result = $this->invoke('assembleColumns', [$rows, [], []]);
        $this->assertSame(
            '"id" bigint GENERATED ALWAYS AS IDENTITY (INCREMENT BY 10 START WITH 100)',
            $result['t']['id']
        );
    }

    /**
     * A column that simply inherits its type's sequence bounds must not have
     * them spelled out, or an `integer` identity and a `bigint` one would
     * differ by their MAXVALUE alone and a type change would read as two.
     */
    public function testAssembleColumnsOmitsIdentityOptionsThatAreTypeDefaults(): void
    {
        $rows = [self::colRow('t', 'id', [
            'data_type'           => 'integer',
            'is_identity'         => 'YES',
            ...
            'identity_start'      => '1',
            'identity_minimum'    => '1',
            'identity_maximum'    => '2147483647',
            'identity_cycle'      => 'NO',
        ])];

        $result = $this->invoke('assembleColumns', [$rows, [], []]);
        $this->assertSame('"id" integer GENERATED ALWAYS AS IDENTITY', $result['t']['id']);
    }

    public function testAssembleColumnsEmitsIdentityCycle(): void
    {
        $rows = [self::colRow('t', 'id', [
            'data_type'           => 'smallint',
            'is_identity'         => 'YES',
            'identity_generation' => 'BY DEFAULT',
            'identity_maximum'    => '999',
            'identity_cycle'      => 'YES',
        ])];

        $result = $this->invoke('assembleColumns', [$rows, [], []]);
        $this->assertSame(
            '"id" smallint GENERATED BY DEFAULT AS IDENTITY (MAXVALUE 999 CYCLE)',
            $result['t']['id']
        );
    }

    /**
     * COLLATE decides comparison and sort order, so a column rendered without
     * it applies cleanly and then orders its rows differently — the quietest
     * kind of wrong this renderer can produce.
     */
    public function testAssembleColumnsEmitsExplicitCollation(): void
    {
        $rows = [self::colRow('t', 'name', [
            'data_type'          => 'text',
            'udt_name'           => 'text',
            'explicit_collation' => 'C',
        ])];

        $result = $this->invoke('assembleColumns', [$rows, [], [], []]);
        $this->assertSame('"name" text COLLATE "C"', $result['t']['name']);
    }

    /** An inherited collation must not be spelled out, or nothing ever matches. */
    public function testAssembleColumnsOmitsInheritedCollation(): void
    {
        $rows = [self::colRow('t', 'name', [
            'data_type'          => 'text',
            'udt_name'           => 'text',
        ...
        $result = $this->invoke('assembleColumns', [$rows, [], [], []]);
        $this->assertSame('"name" text', $result['t']['name']);
    }

    public function testAssembleColumnsEmitsColumnCompression(): void
    {
        foreach (['l' => 'lz4', 'p' => 'pglz'] as $code => $word) {
            $rows = [self::colRow('t', 'big', [
                'data_type'       => 'text',
                'udt_name'        => 'text',
                'att_compression' => $code,
            ])];

            $result = $this->invoke('assembleColumns', [$rows, [], [], []]);
            $this->assertSame("\"big\" text COMPRESSION $word", $result['t']['big']);
        }
    }
