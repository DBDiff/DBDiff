...
class DBDiffComprehensivePostgresTest extends AbstractComprehensiveTest
{
    ...
    /**
     * Partitioned tables were reconstructed as ordinary tables: the parent lost
     * its PARTITION BY clause and each partition became a detached standalone
     * table. Nothing errored — rows still inserted — so the data model quietly
     * changed shape, which is worse than a failure.
     *
     * Asserts on the target's catalog (relkind and relpartbound) rather than
     * the DDL text, because that is what "is it actually partitioned?" means.
     */
    public function testPartitionedTablesArePreserved(): void
    {
        $this->connectTo($this->db1)->exec(
            "CREATE TABLE part_target (
                 id         bigint GENERATED BY DEFAULT AS IDENTITY,
                 order_date date NOT NULL,
                 amount     bigint,
                 PRIMARY KEY (order_date, id)
             ) PARTITION BY RANGE (order_date);
             CREATE TABLE part_target_2024 PARTITION OF part_target
                 FOR VALUES FROM ('2024-01-01') TO ('2025-01-01');
             CREATE TABLE part_target_2025 PARTITION OF part_target
        ...
        $parentKind = $db2->query(
            "SELECT relkind::text FROM pg_class c
               JOIN pg_namespace n ON n.oid = c.relnamespace
              WHERE n.nspname = 'public' AND c.relname = 'part_target'"
        )->fetchColumn();
        $this->assertSame('p', $parentKind, 'Parent must remain a partitioned table');

        $bounds = $db2->query(
            // relkind filter matters: index partitions are also relispartition.
        ...
        $this->assertSame(
            [
                'part_target_2024' => "FOR VALUES FROM ('2024-01-01') TO ('2025-01-01')",
                'part_target_2025' => "FOR VALUES FROM ('2025-01-01') TO ('2026-01-01')",
            ],
            $bounds,
            'Each partition must be attached with its original bound'
        );
    }
