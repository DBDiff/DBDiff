...
class PostgresAdapter implements DBAdapterInterface, BulkSchemaAdapterInterface {
    ...
    public function getViews(Connection $connection): array {
        ...
        foreach ($result as $row) {
            ...
            $sql  = 'CREATE VIEW "' . $row['viewname'] . '"';
            if (!empty($row['options'])) {
                $sql .= ' WITH (' . $row['options'] . ')';
            }
            $views[$row['viewname']] = $sql . ' AS ' . $body;
        }
        return $views;
    }
     ...
     *
     * getViews() reads pg_views, which holds only ordinary views, so these are
    ...
    public function getMaterializedViews(Connection $connection): array {
        ...
        $indexes = $this->materializedViewIndexes($connection);
        $matviews = [];
        foreach ($result as $row) {
            ...
            $sql .= ' AS ' . rtrim(trim($row['definition']), ';');
     ...
     */
    private function materializedViewIndexes(Connection $connection): array {
